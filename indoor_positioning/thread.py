...
class myThread (threading.Thread):
	...
	def run(self):
		IPS(self.b_addr).run_tracking_system(self.name, 20, 1)
...
class IPS:
	...
	def run_tracking_system(self, threadName, counter, delay):
		...
		while True:
			...
			if(rssi > 0):
				node_marker.set_nearest_node(threadName)
			print("Nearest node is ", node_marker.get_nearest_node())
			counter -= 1
